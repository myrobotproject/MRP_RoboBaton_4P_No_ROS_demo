...
typedef struct icm42688_config {
  ...
  /* The C ABI fixes read_mode storage width as uint32_t; currently only
   * ICM42688_READ_MODE_SENSOR_TIMESTAMP_FIFO is accepted. */
  uint32_t read_mode;
  uint32_t reserved[8];
} icm42688_config_t;
...
typedef struct icm42688_sample {
  uint32_t struct_size;
  /* reserved0 is reused as a diagnostic field: the driver's observed count of
   * 20-bit TMST low rollovers. */
  uint32_t reserved0;
  /* host_timestamp_ns is the GPIO395 DRDY edge time converted to the
   * CLOCK_MONOTONIC_RAW time domain before output. */
